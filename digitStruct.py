...
def printHDFObj(theObj, theObjName):
  ...
  print("%s" % (theObjName))
  print("\ttype(): %s" % (type(theObj)))
  if isFile or isGroup or isDataSet:
    print("\tid: %s" % (theObj.id))
  if isFile or isGroup:
    print("\tkeys: %s" % (theObj.keys()))
  if not isReference:
    print("\tLen: %s" % (len(theObj)))
...
def testMain():
  ...
  for dsObj in yieldNextDigitStruct(dsFileName):
    ...
    for bbox in dsObj.bboxList:
      print("\t%s:%d,%d,%d,%d" % (bbox.label, bbox.left, bbox.top, bbox.width, bbox.height))
    if testCounter >= 5:
      break
